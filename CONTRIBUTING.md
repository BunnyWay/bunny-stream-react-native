...
`example-expo/` is a minimal Expo app (Expo SDK 57) that exercises the config-plugin consumer path: video list + VOD player. Its `android/`/`ios/` directories are gitignored and regenerated by Continuous Native Generation, so every clean prebuild is a regression test for `app.plugin.js`.

```bash
# credentials (copy example-expo/.env.example to example-expo/.env)
# EXPO_PUBLIC_BUNNY_ACCESS_KEY / EXPO_PUBLIC_BUNNY_LIBRARY_ID

yarn example:expo prebuild   # generates android/ + ios/, runs pod install
yarn example:expo android    # expo run:android
yarn example:expo ios        # expo run:ios
yarn example:expo start      # dev server (requires a dev build)
```

Plugin changes live in `plugin/src` and compile to `plugin/build` via `yarn plugin:build` (also part of `prepare`) — rebuild before `prebuild` when iterating on the plugin.
